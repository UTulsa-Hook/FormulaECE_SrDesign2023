...
if(doSensorModel)
    ...
    beta_w = 0.1; %bias 
    n_w = randn(1) * 0.5;%zero mean gaussian with a std dev of sigma
    ...
    steering_expect = U(2);
    beta_s = 30;
    n_s = randn(1) * 0.05; %zero mean gaussian with a std dev of sigma
    ...
    heading_expect = X(3);
    beta_h = 0.01;
    n_h = randn(1) * 0.05;  %zero mean gaussian with a std dev of sigma

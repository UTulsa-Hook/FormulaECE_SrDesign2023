...
void AutoCodeTopLevel(void)
{
  ...
  while (1) {
    ...
    insert(queueInsert);
    printf("X: %.2f, Y: %.2f, H: %.2f, V: %.2f, S: %.2f\n", X[0], X[1], X[2], U[0], U[1]);
...
void AutoCodeInitialize(double X[3], double U[2])
{
  X[0] = 9.3; // Testing! Xpos: 9.3 for Circ/HalfTrack, 10.9 for line; 1 for track
  X[1] = 11; // Testing! Ypos: 11 for circ/halftrack, 9.5 for line; 8ish for track
  X[2] = -M_PI; // Testing! heading: pi for circ/halftrack/line, -pi/2 for track
  /*  sample data pwm, speed; */
  /*  sample data pwm, angle */
